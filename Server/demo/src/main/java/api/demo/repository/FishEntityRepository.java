...
public interface FishEntityRepository extends JpaRepository<FishEntity, Long> {
    ...
    @Query(value = "SELECT * FROM fish_products WHERE category = 'food'", nativeQuery = true)
    List<FishEntity> getFishFood();

    @Query(value = "SELECT * FROM fish_products WHERE category = 'grooming'", nativeQuery = true)
    List<FishEntity> getFishGrooming();

    List<FishEntity> findByCategory(String type);
